...
public class GameLifecycleService {
    ...
    /**
     * Soumission du coup reel par le joueur au trait. Retourne l'etat resultant,
     * a diffuser publiquement : le round vient de se resoudre, plus rien a cacher.
     */
    public GameSnapshot submitMove(PlayerToken token, MoveIntent intent) {
     ...
     * trait. intent null signifie "pas de devinette". Ne retourne rien de public :
     * le contenu doit rester cache jusqu'a resolution du round.
     */
    public void submitGuess(PlayerToken token, MoveIntent intent) {
        GameAccess access = requireAccess(token);
        gameRepository.withGame(access.gameId(), game -> {
